# SkyCast Weather Platform
![Pipeline Overview](./src/Overview.png)

## Project Overview
SkyCast delivers a modern, responsive weather experience backed by an end-to-end DevOps toolchain. The frontend provides live conditions, hourly insights, and a six-day outlook for any location using the Open-Meteo APIs. The supporting infrastructure automates build, security, containerization, and multi-environment delivery across AWS.
...
- **Docker**: Containerizes the SkyCast application.
...
- Global location search with graceful error handling and reverse geocoding.
- One-click geolocation detection with privacy-safe fallbacks.
- Animated glassmorphism UI with adaptive color palettes based on forecast conditions.
- Current conditions, vital statistics, hourly strip, and multi-day forecast cards.
...
Use the following Jenkins declarative pipeline (`pipeline_script/build_pipeline`) to build SkyCast. Update the defaults or parameterize to match your Git repository and AWS account.
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'ECR_REPO_NAME', defaultValue: 'skycast-weather', description: 'ECR repository name')
        ...
        stage('2. SonarQube Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    ...
                    -Dsonar.projectName=skycast-weather \\
                    -Dsonar.projectKey=skycast-weather
                    """
                }
            }
        }
...
2. **ArgoCD Bootstrap**: Use `pipeline_script/deployment_pipeline` to configure Prometheus, Grafana, and ArgoCD (defaults updated for SkyCast).
3. **Application Deployments**: ArgoCD tracks the manifests in `k8s_files/` (`skycast-app` deployment & service).
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'CLUSTER_NAME', defaultValue: 'skycast-weather-cluster', description: 'EKS cluster')
    }
...
## Cleanup Pipeline
`pipeline_script/cleanup_pipeline` removes Kubernetes workloads, ArgoCD, Prometheus namespaces, and deletes the SkyCast ECR repository. Update defaults carefully before executing in production environments.
